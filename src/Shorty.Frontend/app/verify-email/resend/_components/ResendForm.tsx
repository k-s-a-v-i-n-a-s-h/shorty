...
export default function ResendForm() {
  const [state, formAction, isPending] = useActionState(resendAction, null);
    ...
    <form action={formAction} noValidate autoComplete="off" className="flex w-full flex-col gap-12">
      ...
      <div className="flex flex-col gap-4 text-base font-medium sm:flex-row">
        <button
          type="submit"
          disabled={isPending}
          className="flex h-12 w-full items-center justify-center gap-2 rounded-full bg-foreground px-5 text-background transition-all duration-200 hover:bg-[#383838] dark:hover:bg-[#ccc] md:w-[158px] disabled:opacity-50 disabled:cursor-not-allowed active:scale-95"        >
